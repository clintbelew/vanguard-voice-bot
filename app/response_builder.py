...
including ElevenLabs voice integration and background ambiance.
"""
...
def enhanced_say(response, text, voice="Polly.Joanna", language="en-US", enhanced=True):
    ...
    try:
        ...
        if enhanced and is_configured():
            logger.info(f"Using ElevenLabs for: {text[:30]}...")
            ...
            if audio_url:
                ...
                logger.warning("Failed to get ElevenLabs audio URL, falling back to Polly")
        
        # Fallback to standard Polly TTS
        logger.info(f"Using standard Polly TTS for: {text[:30]}...")
...
def enhanced_gather(response, text, action, input_types='speech dtmf', 
    ...
    try:
        ...
        # Check if ElevenLabs is configured and enhancement is requested
        if enhanced and is_configured():
            logger.info(f"Using ElevenLabs for gather prompt: {text[:30]}...")
            ...
            if audio_url:
                ...
                logger.warning("Failed to get ElevenLabs audio URL for gather, falling back to Polly")
        
        # Fallback to standard Polly TTS
        logger.info(f"Using standard Polly TTS for gather prompt: {text[:30]}...")
