...
def voice():
    ...
    try:
        ...
        response = VoiceResponse()
        response.say(ERROR_MESSAGES['general'], voice='Polly.Joanna')
...
def handle_response():
    ...
    try:
        ...
        elif any(phrase in speech_result for phrase in ['appointment', 'schedule', 'book']):
        ...
        # Create a fallback response
        response = VoiceResponse()
        response.say(ERROR_MESSAGES['input_not_understood'], voice='Polly.Joanna')
...
def handle_appointment():
    ...
    try:
        ...
        if any(word in speech_result for word in ['yes', 'yeah', 'sure', 'tomorrow', 'friday', '10', '2']):
            response.say("Great! I've scheduled your appointment. You'll receive a confirmation message shortly.", voice='Polly.Joanna')
            response.say("Is there anything else I can help you with today?", voice='Polly.Joanna')
            response.redirect(url_for('main.voice'))
        else:
            response.say("I understand those times don't work for you. Let me connect you with our scheduling team to find a better time.", voice='Polly.Joanna')
