...
def create_clean_audio_url(filename):
    """
    Create a clean, properly formatted audio URL using the PUBLIC_URL_BASE environment variable.
    This function directly constructs the URL rather than trying to fix malformed ones.
    
    Args:
        filename: The audio filename (without path)
        
    Returns:
        A properly formatted URL for the audio file
    """
...
def get_cached_audio_url(text, language="en-US"):
    ...
    if os.path.exists(cache_file):
        ...
        # Return the public URL for this cached file using the new clean URL function
        return create_clean_audio_url(f"{audio_hash}.mp3")
