...
def normalize_url(url):
    """Normalize URL to ensure it doesn't have duplicate schemes or domains."""
    ...
    if 's://' in parsed.netloc or '://' in parsed.path:
        logger.warning(f"Found malformed URL with duplicate schemes: {url}")
        ...
        logger.info(f"Normalized URL from {url} to {clean_url}")
        return clean_url
...
def get_cached_audio_url(text, language="en-US"):
    ...
    if os.path.exists(cache_file):
        ...
        url = f"{PUBLIC_URL_BASE}/audio/{encoded_filename}"
        # Normalize the URL to prevent malformed URLs
